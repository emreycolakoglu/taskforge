...
describe('DetailAssigneeSelect', () => {
  it('shows the selected user in the trigger', () => {
    render(<DetailAssigneeSelect value="u1" users={users} onChange={() => {}} />);
  ...
  it('maps the Unassigned option back to null', async () => {
    ...
    render(<DetailAssigneeSelect value="u1" users={users} onChange={onChange} />);
  ...
  it('passes a picked user id through unchanged', async () => {
    const user = userEvent.setup();
    const onChange = vi.fn();
    render(<DetailAssigneeSelect value={null} users={users} onChange={onChange} />);
    await user.click(screen.getByRole('combobox'));
