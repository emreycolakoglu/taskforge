                ...
                <CommandGroup heading="Tasks">
                  {tasks.map((task) => (
                    <CommandItem
                      ...
                      onSelect={() => handleSelect('task', task.id, task.boardId)}
                    >
                      <Search className="size-3.5 text-muted-foreground shrink-0" />
                      <span className="shrink-0 font-mono text-xs text-muted-foreground w-16">
                        {task.taskNumber}
                      </span>
                      <span className="truncate text-foreground flex-1 text-sm">{task.title}</span>
                      <span
                        className={cn(
                ...
                <CommandGroup heading="Boards">
                  {filteredBoards.map((board) => (
                    ...
                    >
                      ...
                      <span className="text-base leading-none shrink-0">{board.icon ?? '⭐'}</span>
                      <span className="truncate text-foreground flex-1 text-sm">{board.name}</span>
                      {board.identifier && (
