    ...
    >
      ...
      {hasRow2 && (
        <div className="flex items-center gap-1.5 text-muted-foreground text-xs min-w-0">
          {parentTaskNumber && (
            <span className="font-mono text-[10px] text-muted-foreground/70 bg-muted rounded px-1 py-0 whitespace-nowrap shrink-0">
              ↳ {parentTaskNumber}
            </span>
          )}
          ...
          {visibleLabels.length > 0 && (
            <div className="flex items-center gap-1 shrink-0">
              {visibleLabels.map((tl) => (
                <span
                  key={tl.labelId}
                  className="inline-flex items-center rounded-sm border border-black/10 px-1 py-0 text-[10px] font-medium leading-4 whitespace-nowrap"
                  style={{
