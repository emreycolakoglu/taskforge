...
async function main() {
  ...
  const feature = await prisma.label.create({
    data: { boardId: board.id, name: 'Feature', color: '#22C55E' },
  });
  const docs = await prisma.label.create({
    data: { boardId: board.id, name: 'Documentation', color: '#3B82F6' },
  });
  const urgent = await prisma.label.create({
    data: { boardId: board.id, name: 'Urgent', color: '#F97316' },
  });
  ...
  const t3 = await prisma.task.create({
    data: {
      ...
      priority: 'urgent',
      assigneeId: alice.id,
      description: 'Add email/password login, session management, and invite-based signup flow.',
    },
  });
  ...
  const t5 = await prisma.task.create({
    data: {
      ...
      boardId: board.id,
      number: 5,
      title: 'Review PR #142 — API refactor',
      position: 0,
      priority: 'high',
      description:
        'Major refactor of the task service layer. Needs careful review of the new optimistic update logic.',
    },
  });
    ...
    data: [
      {
        ...
        authorId: alice.id,
        author: 'Alice',
        body: 'Need OAuth2 providers configured first.',
      },
      {
        taskId: t3.id,
        authorId: bob.id,
        author: 'Bob',
        body: 'I can help with the Google provider setup. I have experience with Passport.js.',
      },
      {
        taskId: t4.id,
        authorId: charlie.id,
        author: 'Charlie',
        body: 'Testing on Safari now — looks good. The CSS variables approach works well.',
      },
      {
        taskId: t5.id,
        authorId: alice.id,
        author: 'Alice',
        body: 'Left some comments on the optimistic update logic. The rollback path looks solid.',
      },
    ],
  });
    ...
    data: [
      {
        ...
        actor: 'Alice',
        action: 'created',
        detail: JSON.stringify({ title: t3.title }),
      },
      {
        taskId: t3.id,
        actorId: bob.id,
        actor: 'Bob',
        action: 'commented',
        detail: JSON.stringify({ commentId: 'seed' }),
      },
      {
        taskId: t4.id,
        actorId: charlie.id,
        actor: 'Charlie',
        action: 'created',
        detail: JSON.stringify({ title: t4.title }),
      },
      {
        taskId: t5.id,
        actorId: alice.id,
        actor: 'Alice',
        action: 'moved',
        detail: JSON.stringify({ to: 'Review' }),
      },
    ],
  });
