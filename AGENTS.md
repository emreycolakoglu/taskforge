...
- **The service worker is cosmetic — it exists only to keep the PWA registration alive.** `src/sw.js` (built via `injectManifest`, `globPatterns: []`) deliberately adds **no `fetch` handler**, so every request — including `/api` and `/ws` — goes straight to the network exactly as if there were no worker. Do **not** add precaching, `runtimeCaching`, or a `fetch` handler: it would reintroduce the white-screen bug where a stale worker answered hashed-chunk requests with a cached `index.html`, which the module loader rejects. `src/sw.test.ts` enforces this contract.
- **`registerType: 'autoUpdate'`** — because the worker never serves anything, an update only means the worker's own bytes changed; reloading then is harmless and keeps the app from sitting on stale logic for days.
...
- **Cache headers are deliberate** (`apps/api/src/main.ts`): `index.html`, `sw.js`, `registerSW.js` and `manifest.webmanifest` are served `no-store` so a redeploy is picked up immediately; everything under `/assets/` is content-addressed and served `immutable`. The SPA fallback 404s missing `/assets/*` instead of returning the HTML shell — a 200 `text/html` for a `.js` request is what makes the module loader fail.
